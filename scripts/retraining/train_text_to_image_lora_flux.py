...
"""Fine-tuning script for FLUX diffusion models for text2image with LoRA support.
...
def main(args):
    ...
    if args.lora_layers is not None:
        ...
        target_modules = [
            # Attention block projections (pruned by linear_layer_pruning)
            "attn.to_k",
            "attn.to_q",
            "attn.to_v",
            "attn.to_out.0",
            ...
            # FFN block projections (pruned by ffn_linear_layer_pruning)
            "ff.net.0.proj",
            "ff.net.2",
            ...
            # Norm projections (pruned by norm_layer_pruning)
            "norm1_context.linear.sparse_proj" if args.pruned_transformer_model_path is not None else "norm1_context.linear",
            "norm1.linear.sparse_proj" if args.pruned_transformer_model_path is not None else "norm1.linear",
            "norm.linear.sparse_proj" if args.pruned_transformer_model_path is not None else "norm.linear",
        ]
    ...
    if accelerator.is_main_process:
        tracker_name = "text2image-flux-dev-lora"
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        '''if args.pruned_transformer_model_path is not None:
                           transformer = unwrap_model(transformer)
                            lora_state_dict = convert_state_dict_to_diffusers(get_peft_model_state_dict(transformer))
                            file_name = args.pruned_transformer_model_path.split("/")[-1]
                            with open(
                                os.path.join(
                                    "/".join(args.pruned_transformer_model_path.split("/")[:-1]), f"lora_{file_name}"
                                ),
                                "wb",
                            ) as f:
                                pickle.dump(lora_state_dict, f)'''
                        ###### save the pruned transformer model ######
